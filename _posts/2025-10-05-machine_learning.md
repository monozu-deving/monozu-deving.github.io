...
안녕하세요, 오늘은 흔히 인공지능하면 나오는 머신러닝에 대해서 설명을 해보려고 합니다.

# Machine Learning이란
우선 일반적으로 Machine Learning을 분류할 때는 크게 지도 학습(Supervised Learning), 비지도 학습(Unsupervised Learning), 강화 학습(Reinforcement Learning)이 있습니다. 즉 해당 학습 방법들이 Machine Learning 내에 속해있다고 보면 됩니다.

그러면 해당 3가지 분류에 대해서 아래 내용에서 설명드리도록 하겠습니다.

## Supervised Learning(지도학습)
우선 Supervised Learning(지도 학습)같은 경우에는 입력(input)과 정답(output)을 제공해주고 이를 기반으로 학습을 진행합니다.

이런 학습 방식은 새롭게 보지 못한 입력에 대해 정확한 출력 예측이 목표이며, 예를 들어서 사과 사진을 넣어서 사과라고 라벨링을 해주고 학습을 하게 된다면 어떤 사과 사진을 넣어도 사과라고 답할 수 있게 되는 것이 지도 학습의 목표입니다.

주로 Regression(회귀)와 Classification(분류)로 나뉘는데 회귀는 수치를 넣어서 어떤 사이즈의 생선의 가격은 얼마일 것이다 라고 수치에 대한 값을 예측하는 것이고, 분류는 이게 어떤 사이즈이니 어떤 생선일 것이라고 종류를 맞추는 것이라고 볼 수 있습니다. 

정답과 입력을 제공하는 지도 학습에는 어떤 것이 있느냐? 라고 한다면 일반적으로 DNN, CNN, RNN과 같은 것들이 이에 해당합니다. (각각이 어떤 기능을 하는지도 역시 추가적으로 post를 작성하도록 하겠습니다.)

저런 친구들을 활용해서 Object Detection, Pose Estimation, Segmentation과 같은 기능들을 할 수 있는데 Object Detection과 Segmentation과 같은 경우에는 라벨 데이터를 제공해줌으로서 어떤 라벨인지 알 수 있게 활용하기 때문에 지도 학습에 해당합니다.

지도 학습을 일반적으로 많이 사용하는 가장 큰 이유는 Transfer Learning(전이 학습)이 가능한 것 때문이라고도 볼 수 있는데, 전이 학습 같은 경우에는 사전에 학습된 Weight의 값을 가져오는 것으로, 이미 잘 학습된 가중치를 가져와서 기존 신경망을 파인튜닝 한 자신의 모델에 사용할 수도 있습니다.  
(물론 이걸 쓴다고 정확도가 올라간다고 장담은 할 수 없지만 원하는 결과에 fitting하는데 도움이 될 수 있습니다. => 이러면 컴퓨터 리소스를 적게 사용해도 Gradient가 이상한 방향으로 안갈 수도 있기 때문에 일반적으로 yolo와 같은 모델들은 대부분 파인 튜닝을 해서 사용하는 사람들이 많이 존재합니다.) 

## Unsupervised Learning(비지도 학습)
지도 학습과 다르게 일반적으로 비지도 학습은 label이 없습니다. 
```
Q: 그러면 어떻게 학습하나요?  
A: ~~잘~~ 
```
비지도 학습은 사람이 어떻게 관여를 하기 힘든 구조를 가지고 있어, 컴퓨터가 맘대로 학습을 진행합니다. (엄밀히 말하자면 라벨 없이도 정의된 손실함수를 기반으로 스스로 패턴을 찾는 방식이라고 볼 수 있습니다.)

즉, 어떤 데이터들이 주어졌을때 label이 없어도 데이터 간의 유사도나 분포를 기반으로 스스로 구조를 학습한다. 이 과정에서 모델은 서로 비슷한 데이터의 벡터는 가깝게, 다른 특성을 가진 벡터는 멀어지도록 표현 공간을 형성한다고 볼 수 있습니다.
물론 사람이 지정한 식에 따라서 가는 것이기 하지만 특정 패턴을 찾아서 컴퓨터가 분류를 해주는 것이라고 볼 수 있습니다. 

이러한 이유에 있어서 비지고 학습은 사람이 찾지 못한 데이터 내의 숨겨진 구조와 패턴 분포를 찾을 수 있습니다. 해당 특징 덕분에 스스로 그룹화하거나 특징을 추출하기 때문에 No Feedback, 피드백을 제공해주지 않아도 됩니다. 

또한 비지도 학습으로 학습된 데이터들은 지도 학습에 사용될 수 있습니다. 비지도 학습으로 분류된 것들에 label만 붙여주게 된다면 지도 학습에 사용할 수 있다는 말이 됩니다.

Clustering과 K-means, PCA, t-SNE, AutoEncoder 등이 이에 해당하며 이에 대해서는 나중에 추가로 post를 작성하도록 하겠습니다. 

## Reinforcement Learning(강화 학습)
기존의 지도 학습과 비지도 학습은 다르게 강화 학습은 행동(Action)을 통해 환경(Environment)과 상호작용을 하면서 보상(Reward)을 기반으로 학습합니다. 비행기 게임을 예로 들어서 아무것도 모르는 아기가 비행기 게임을 시작했을 때 적에게 맞으면 사탕을 잃고, 미사일로 적 비행기를 격추하면 사탕을 주는 방식과 유사하다고 볼 수 있습니다. 여기서 보스 비행기를 죽이면 대왕 사탕을 주는 등의 방식으로 Reward를 조절할 수 있습니다.

이를 통해서 최적의 행동 정책(Policy)을 학습해 누적 보상(Maximum Reward)을 극대화해내는 방향으로 학습합니다.
여기서 아기, 일반적으로 학습자(Agent)는 시행착오를 통해 배우게 됩니다. 

이러한 이유에서 강화 학습은 여러 번 도전이 필요하며 이 과정에서 샘플 효율성이 낮고, 보상 설계를 잘 해야 정확한 방향으로 나아갈 수 있습니다. 로봇 측면에서 보았을 때는 물리적으로 학습을 제대로 안정적이게 할 수 있는가?가 현실적으로 문제가 되는 모습을 보이며 이를 해결하기 위해 Imitation Learning이라는 방식이 제시었고, 이는 나중에 자세히 다루도록 하겠습니다.  


## 결론
결과적으로 Machine Learning이라는 것에 Supervised Learning과 Unsupervised Learning, Reinforcement Learning이 속하는 것으로 이들은 아래의 표와 같은 차이를 가집니다.  
