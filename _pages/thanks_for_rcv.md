---
layout: thanks_for_rcv
title: "Thanks_For_RCV"
permalink: /thanks_for_rcv/
description: "이름과 개인 코드로 편지를 확인하는 페이지"
sitemap: false
search: false
---

RCV의 여러분들에게 김기현이 남긴 편지입니다.

이름과 개인 코드를 입력하면 7월 10일 이후 편지 내용을 열람하실 수 있습니다.

<p class="thanks-reply">
  답장은
  <span class="thanks-reply__channel">
    <img src="{{ '/assets/images/thanks-for-rcv/slack.png' | relative_url }}" alt="" aria-hidden="true" loading="lazy" decoding="async">
    Slack
  </span>이나
  <a class="thanks-reply__channel thanks-reply__channel--instagram" href="https://www.instagram.com/airo._.hyeon" target="_blank" rel="noopener noreferrer">
    <img src="https://cdn.simpleicons.org/instagram/E4405F?viewbox=auto&amp;size=18" alt="" aria-hidden="true">
    Instagram
  </a>으로 부탁드립니다.
</p>
